...
int map_file_to_memory(char** mapped_file, int file_descriptor, int file_size){
    *mapped_file = mmap(NULL, file_size, PROT_READ | PROT_WRITE, MAP_SHARED, file_descriptor, 0);
...
int invert_file_content(char** mapped_file, int file_size){
    int byte_size = sizeof(char);
    int half_file_size;
    if(file_size%2*byte_size != 0){
        half_file_size = (file_size/2)*byte_size + byte_size;
    } else {
        half_file_size = (file_size/2)*byte_size;
    }
    for (int i = 0; i < half_file_size; i+=byte_size){
        char temp = (*mapped_file)[i];
        (*mapped_file)[i] = (*mapped_file)[file_size - i - byte_size];
        (*mapped_file)[file_size - i - byte_size] = temp;
    }
    return 0;
}
...
int main(){
    ...
    invert_file_content(&mapped_file, file_size);

    printf("Inverted mapped file content:\n%s\n\n", mapped_file);
